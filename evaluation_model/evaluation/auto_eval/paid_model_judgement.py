# -*- coding: utf-8 -*-
"""
心理咨询模型评估脚本（商用API版；仅接受严格 JSON 输出；评测可复现）
输出：
1) all_model_outputs.json —— 所有样本及模型输出（含对话与标准答案）
2) 控制台直接 print 每个“模型配置”的分数（不再写 evaluation_metrics_summary.json）
...
API_KEY = "sk-x"              # ← 在这里放你的真实 Key
MODEL = "gpt-4.1"
TEMPERATURE = 0.1
MAX_TOKENS = 32               # 每次生成 token 上限
TIMEOUT = 30                  # seconds
# =============================================================
...
BACKOFF_BASE = 1.0
BATCH_SIZE = 64
USE_SYSTEM = True            # 是否保留样本中的 system 提示
SCALE_RANGE = (0, 3)          # 量表取值范围
TEST_PATH = "eval_data_judgment.json"  # 你的测试集路径
# =============================================================
...
def extract_json_from_response(response: Optional[str]):
    if not response:
        return None
...
def get_last_assistant_content(messages: List[Dict[str, str]]):
...
# 生成输入构造 / 清理
# ===============================
def clean_generated_response(response: Optional[str]) -> str:
    return response.strip() if response else ""


def build_messages_with_optional_system(messages: List[Dict[str, str]], use_system: bool = True):
    """
...
# ===============================
# API 调用（OpenAI Chat Completions 兼容）
# ===============================
def _request_with_retries(url: str, headers: Dict[str, str], payload: Dict[str, Any],
                          max_retries: int = MAX_RETRIES, timeout: int = TIMEOUT) -> Dict[str, Any]:
        ...
        try:
            ...
            if resp.status_code >= 400:
                try:
                    ...
                    err_j = resp.text
                raise RuntimeError(f"HTTP {resp.status_code}: {err_j}")
            return resp.json()
            ...
            if attempt < max_retries - 1:
                ...
                continue
            raise RuntimeError(f"API request failed after {max_retries} attempts: {last_err}") from last_err


def call_chat_api(msg_list: List[Dict[str, str]]) -> str:
    """
    以 OpenAI Chat Completions 兼容格式调用。返回 content(str)。
    若使用 Gemini 的“OpenAI 兼容端点”，保持同样可用。
    """
    if not API_KEY or API_KEY == "sk-x":
        raise RuntimeError("请在本文件顶部把 API_KEY 设置为你的真实 Key。")
    ...
    payload = {
        ...
        "messages": [{"role": m["role"], "content": m["content"]} for m in msg_list],
        "response_format": {"type": "json_object"},  # 强制返回 JSON
    }
    j = _request_with_retries(API_BASE_URL, headers, payload)
    return j["choices"][0]["message"]["content"]


# ===============================
# 批量生成（走商用 API；保留一次“修复重试”）
# ===============================
def batch_generate_api(
    ...
    for i in tqdm(range(0, len(test_data), batch_size), desc="Processing batches"):
        ...
        for msgs in batch_messages_list:
            # —— 第一次生成 —— #
            try:
                resp1 = call_chat_api(msgs)
            except Exception:
                resp1 = ""

            resp1 = clean_generated_response(resp1)
            parsed = extract_json_from_response(resp1)
            if parsed is not None:
                finalized.append(resp1)
            else:
                # —— 修复重试（极短、极硬提示） —— #
                ...
                try:
                    resp2 = call_chat_api(repair_msgs)
                except Exception:
                    resp2 = ""
...
def main():
    ...
    if not API_KEY or API_KEY == "sk-x":
        raise RuntimeError("请在本文件顶部把 API_KEY 设置为你的真实 Key（当前为占位符 sk-x）。")
